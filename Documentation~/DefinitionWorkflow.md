# Core-State: definition workflow
...
3. Use its buttons to exercise add typed item, remove current item.
4. Inspect the configured hosts and triggers, then open
   [CoreStateWorkflow.cs](../Samples~/DefinitionWorkflow/Runtime/CoreStateWorkflow.cs). It is the caller
...
- **Add typed item**: `CoreStateWorkflow.Add()`.
- **Remove current item**: `CoreStateWorkflow.Remove()`.
