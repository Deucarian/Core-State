# Deucarian Core State Agent Notes

Package ID: `com.deucarian.core-state`
Repository: `Deucarian/Core-State`
...
- Generic repositories, selection service primitives, and state contracts.
...
- UI binding, world selection, logging unless needed, Unity object ownership, or domain-specific state.

## Dependencies

Allowed dependency shape:

- No dependencies by default. Add a dependency only if the primitive layer directly uses that capability.
...
- Logging: No Logging dependency unless a future audited need is proven.
- Common: No Common dependency unless direct Unity object cleanup is introduced, which should be unusual for this package.
- Editor UI: No editor shell ownership.
- Diagnostics: No diagnostics ownership.
- Testing: Tests should remain plain repository/selection behavior tests.
